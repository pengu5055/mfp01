...
# plt.plot(x_lin1, np.abs(ai1 - arrayize(x_lin1, airyai))/ai1, c="#ffc07f", label="Ai(x)")
plt.plot(x_lin1, np.abs(bi1 - arrayize(x_lin1, airybi))/bi1, c="#dac4f7", label="Bi(x)")
# plt.plot(x_lin2, np.abs(ai2 - arrayize(x_lin2, airyai))/ai2, c="#ffc07f")
plt.plot(x_lin2, np.abs(bi2 - arrayize(x_lin2, airybi))/bi2, c="#dac4f7")
# plt.plot(x_lin3, np.abs(ai3 - arrayize(x_lin3, airyai))/ai3, c="#ffc07f")
plt.plot(x_lin3, np.abs(bi3 - arrayize(x_lin3, airybi))/bi3, c="#dac4f7")
plt.legend()
plt.title("Relativna napaka vgrajenih funkcij Bi(x)")
plt.xlabel("x")
plt.ylabel(r"$|Bi_{Sci} - Bi_{ref}|/Bi_{Sci}$")
plt.yscale("log")
plt.show()
